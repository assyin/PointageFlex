  ...
  useEffect(() => {
    if (config) {
      reset({
        ...
        notifyMissingIn: (config as any).notifyMissingIn ?? true,
        notifyMissingOut: (config as any).notifyMissingOut ?? true,
        notifyLate: (config as any).notifyLate ?? true,
        notifyAbsence: (config as any).notifyAbsence ?? true,
        notifyAbsencePartial: (config as any).notifyAbsencePartial ?? true,
        notifyAbsenceTechnical: (config as any).notifyAbsenceTechnical ?? true,
        notifyOvertimePending: (config as any).notifyOvertimePending ?? true,
      });
    }
  }, [config, reset]);
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <div>
                      <p className="font-medium text-blue-900">OVERTIME_PENDING</p>
                      <p className="text-xs text-blue-700">Heures sup. en attente</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        {...register('notifyOvertimePending')}
                        className="sr-only peer"
                      />
                      <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                    </label>
