...
function generateMatricule(departmentCode: string, functionCode: string, index: number, type: 'EMP' | 'MGR' | 'MGR_REG' = 'EMP') {
...
function generateEmail(matricule: string, tenantSlug: string) {
...
async function main() {
  ...
  if (shiftSoir) console.log(`   Soir: ${shiftSoir.name} (${shiftSoir.startTime} - ${shiftSoir.endTime})`);
  if (shiftNuit) console.log(`   Nuit: ${shiftNuit.name} (${shiftNuit.startTime} - ${shiftNuit.endTime})`);
  console.log('');

  // 2. Vérifier et créer les départements
  ...
  // 3. Créer les fonctions/postes
  console.log('💼 Création des fonctions/postes...');
  const positionMap = new Map<string, string>(); // "DEPT-FUNC" -> id
  ...
  const employeeMap = new Map<string, { id: string; departmentId: string; siteId: string }>();
  ...
  for (const [deptName, functions] of Object.entries(DEPARTMENT_FUNCTIONS)) {
    ...
    // Créer 1 manager de département
    ...
    if (mgrEmployee) {
      ...
      // Créer ou récupérer l'utilisateur manager
      ...
      // Assigner le rôle MANAGER (si pas déjà assigné)
      ...
      mgrEmployee = await prisma.employee.create({
        data: {
          ...
          departmentId: deptId,
          siteId: tenant.sites[0].id, // Premier site par défaut
    ...
    // Assigner comme manager du département
    ...
    // Créer 1 manager régional par site
    for (let siteIndex = 0; siteIndex < tenant.sites.length; siteIndex++) {
      ...
      if (mgrRegEmployee) {
        ...
        // Créer SiteManager (si pas déjà existant)
    ...
    // Pour chaque fonction, créer 7 employés par site
    for (const funcName of functions) {
  ...
  for (const [key, emp] of employeeMap.entries()) {
    // Déterminer le shift selon le département
    let shiftId = shiftMatin.id;
    if (key.includes('CPT-') && !key.includes('MGR')) {
      // CPT travaille sur différents shifts (sauf les managers)
      const shifts = [shiftMatin, shiftSoir, shiftNuit].filter(s => s !== null) as typeof shiftMatin[];
      if (shifts.length > 0) {
        shiftId = shifts[Math.floor(Math.random() * shifts.length)]!.id;
      }
    }

    // Générer les dates (exclure les jours fériés)
    const dates: Date[] = [];
    ...
    while (current <= scheduleEnd) {
      ...
      // Exclure dimanche et jours fériés
    ...
    // Créer un planning aléatoire (80% de présence pour les employés, 100% pour les managers)
  ...
  for (const [key, emp] of employeeMap.entries()) {
    ...
    for (const schedule of schedules) {
      ...
      // Cas possibles pour les pointages
      const scenarios = [
      ...
      // Créer pointage OUT (sauf si sortie manquante)
      if (!selectedScenario.missingOut) {
        ...
        // Si le shift traverse minuit, ajouter un jour
