/**
 * Script de test complet pour les 21 scénarios de notifications email
 *
 * Ce script teste tous les types d'anomalies et leurs notifications:
 * - LATE (5 scénarios)
...
async function getTestEmployee() {
  ...
  if (!employee) throw new Error('Aucun employé de test trouvé');
  return employee;
}

async function getShift(startTime: string = '08:00') {
  const shift = await prisma.shift.findFirst({
    where: { tenantId: TEST_TENANT_ID, startTime },
  });

  if (!shift) throw new Error(`Aucun shift trouvé avec startTime=${startTime}`);
  return shift;
}

async function getOrCreateSchedule(employeeId: string, shiftId: string, date: Date) {
...
async function testLate01_LateDetected() {
  ...
  try {
    ...
    // Créer un pointage avec 25 min de retard (> seuil de 14 min)
...
async function testLate02_LateUnderThreshold() {
  ...
  try {
    ...
    // Créer un pointage avec 10 min de retard (< seuil de 14 min)
    const lateTime = new Date(today);
    ...
    await prisma.attendance.create({
      data: {
        ...
        type: AttendanceType.IN,
        method: DeviceType.MANUAL,
        hasAnomaly: false, // Pas d'anomalie car sous le seuil
...
async function testLate05_MultipleNotifications() {
  ...
  try {
    ...
    // Vérifier le nombre de notifications LATE aujourd'hui
...
// ============================================
// TESTS ABSENCE_PARTIAL (5 scénarios)
// ============================================
...
async function testAbsencePartial01_Detected() {
  ...
  try {
    ...
    // Créer un pointage IN tardif (3h de retard = absence partielle)
    const lateTime = new Date(today);
...
async function testAbsencePartial02_UnderThreshold() {
  ...
  try {
    ...
    // Créer un pointage avec 1h30 de retard (< seuil de 2h)
    const lateTime = new Date(today);
    ...
    await prisma.attendance.create({
      data: {
        ...
        method: DeviceType.MANUAL,
        hasAnomaly: true,
        anomalyType: 'LATE', // Traité comme LATE, pas ABSENCE_PARTIAL
        lateMinutes: 90,
...
// ============================================
// TESTS ABSENCE_TECHNICAL (5 scénarios)
// ============================================
...
async function testAbsenceTechnical01_MissingOut() {
  ...
  try {
    ...
    // Créer un IN sans OUT correspondant (session ouverte)
...
async function testAbsence01_FullAbsence() {
  ...
  try {
    ...
    // Vérifier s'il y a des absences aujourd'hui
