/**
 * Script de test simplifié pour les notifications email
 * Crée des données de test et vérifie les résultats
 */
...
async function main() {
  ...
  try {
    ...
    if (!employee) throw new Error('Aucun employé trouvé');

    const shift = await prisma.shift.findFirst({
      where: { tenantId: TEST_TENANT_ID, startTime: '08:00' },
    });

    if (!shift) throw new Error('Aucun shift trouvé');
    ...
    // 2. Créer des données de test pour aujourd'hui
    ...
    // 3. Créer un pointage LATE (retard de 25 min)
